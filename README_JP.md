...
| Top K | 1 | 候補トークン数。1は貪欲法となりTop PとTemperatureは無効。0以下ならサンプラー設定をLiteRT-LMに任せる |
| Top P | 0.95 | 累積確率のしきい値。Top Kが2以上のときのみ有効 |
| Temperature | 1 | 大きいほど回答がばらつき、小さいほど安定する。Top Kが2以上のときのみ有効 |
| Randomize Seed | オン | リクエストごとに新しいシードを引く。同じ質問でも異なる回答になる。Top Kが2以上のときのみ有効 |
| Seed | 0 | 固定のシード。サンプリングが再現可能になる。Randomize Seedがオフで、Top Kが2以上のときのみ有効 |
...
リクエストごとに新しいConversationを作るため、サンプリングは毎回シードから再開します。同じ質問で異なる回答が欲しい場合はRandomize Seedをオンのままに、まったく同じ回答を再現したい場合はオフにしてSeedを固定してください。
